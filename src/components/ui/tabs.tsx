...
const tabListVariants = cva(
  "relative flex w-full p-1 text-muted-foreground data-[orientation=vertical]:flex-col data-[orientation=horizontal]:items-center data-[orientation=vertical]:items-stretch",
  {
    variants: {
      variant: {
        block:
...
export const TabsList = <T extends ValidComponent = "div">(
  props: PolymorphicProps<T, tabsListProps<T>>,
	...
	const [local, rest] = splitProps(props as tabsListProps, ["class","variant"]);

	return (
		<TabsPrimitive.List
      class={cn(tabListVariants({variant: local.variant}), local.class)}
			{...rest}
		/>
	);
};
...
const tabsTriggerVariants = cva(
  "peer relative z-10 inline-flex h-7 w-full items-center justify-center whitespace-nowrap rounded-md py-1 px-3 text-sm font-medium outline-none transition-colors disabled:pointer-events-none disabled:opacity-50 gap-[0.4rem]",
  {
    variants: {
      variant: {
        block:
          "data-[selected]:text-foreground px-3",
        underline:
          "px-4 data-[selected]:text-primary",
      },
    },
    defaultVariants: {
      variant: "block",
...
export const TabsTrigger = <T extends ValidComponent = "button">(
	...
	const [local, rest] = splitProps(props as tabsTriggerProps, ["class", "variant"]);
